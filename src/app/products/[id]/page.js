...
export default async function ProductDetails({ params }) {
    ...
    // Connect to DB and fetch product directly
    const collection = await dbConnect("products");
    // const product = await collection.findOne({ _id: new ObjectId(id) });
    // After fetching the product
    const productRaw = await collection.findOne({ _id: new ObjectId(id) });

    if (!productRaw) return <p>Product not found.</p>;
    ...
    return <ProductDetailsClient key={product._id} product={product}></ProductDetailsClient>
}
